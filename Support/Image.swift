...
public struct Image {
    ...
    public init(jpeg url: URL, byteOrdering: ByteOrdering = .rgb) {
        ...
        if (byteOrdering == .bgr) {
            self.imageData = .uint8(data: Raw.reverse(loadedJpeg, dims: Tensor<Bool>([false, false, false, true])))
        } else {
    ...
    public func save(to url: URL, quality: Int64 = 95) {
        // Detect tensor channels, save in grayscale if one, RGB if 3, RGBA if 4
//        let format = .rgb
//        let format = TensorFlow.Format.grayscale
        ...
        let encodedJpeg = Raw.encodeJpeg(image: outputImageData, format: .grayscale, quality: quality, xmpMetadata: "")
        Raw.writeFile(filename: StringTensor(url.absoluteString), contents: encodedJpeg)
    }
    
    public func resized(to size: (Int, Int)) -> Image {
        switch self.imageData {
        case let .uint8(data): return Image(tensor: Raw.resizeBilinear(images: Tensor<UInt8>([data]), size: Tensor<Int32>([Int32(size.0), Int32(size.1)])))
        case let .float(data): return Image(tensor: Raw.resizeBilinear(images: Tensor<Float>([data]), size: Tensor<Int32>([Int32(size.0), Int32(size.1)])))
        }
        
    }
}

// TODO: Input of a grid of images as a 4th-rank tensor
